    ...
    def connect(self):
        while True:
            ...
            try:
                ...
                # 如果没有抛出异常说明连接成功，直接返回
                self.msg_log.push('连接%s成功' % self.server)
                # 接收到的网络数据解码为str
            ...
            # 直接返回失败不定位
                ...
                if self.try_times != 0:
                    self.msg_log.push('连接%s失败，正在重新连接' % self.server)
                    self.try_times -= 1
                else:
                    self.msg_log.push('连接服务器失败，请检查用户名密码是否正确，目标服务器是否在线。')
...
class TestPerform(object):
    def __init__(self, filename,queue1,queue2):
    ...
    def case_run(self):
        self.queue_log.push('用例路径：' + self.filename)
        # for循环根据case-step进行类初始化，每次把所有step的ssh都先连接上，再进行操作；
        # 执行测试部分
        for case in self.case_index:
            # case内操作
            class_list = []
            self.queue_log.push('START TESTING CASE: %s , %s' % (case,time.strftime("%Y.%m.%d %H:%M:%S", time.localtime())))
            ...
            for i in range(len(self.case_step_dict[case])):
                class_list[i].close()
            self.queue_log.push('CASE ENDING: %s , %s' % (case,time.strftime("%Y.%m.%d %H:%M:%S", time.localtime())))
    ...
    def case_read(self):
        df = pd.read_excel(self.filename)  # 读取xlsx中第一个sheet
        for i in df.index.values:   # 获取行号的索引，并对其进行遍历：
            try:
                ...
                self.queue_log.push('用例表格内容有误，请检查！')
                self.queue_log.push(KeyError_msg)
...
# 这是个啥
class running_log_x(QThread):
    running = pyqtSignal(str)

    def __init__(self, parent=None):
        super(running_log_x, self).__init__(parent)
        # 设置工作状态与初始num数值
        self.working = True
        self.num = 0
...
#   读取消息队列内消息，实现消息打印&写log类，需要加一步推送数据到UI指定位置的功能
class Logging(object):
    ...
    def __init__(self, filename, queue1, queue2):
        ...
        self.queue_status = queue2
        self.msg = ''
        # 再增加一个推送数据到UI界面对应文本框的动作

    # 这两个函数好像可以直接在UI模块那边使能读队列线程，并打印到对应窗口里
    def log_print(self):
        while True:
            self.msg = (self.queue_log.get())
            self.file_log.write(self.msg + '\n')
            print(self.msg)

    def status(self):
        while True:
            self.msg = (self.queue_status.get())
            self.file_log.write(self.msg + '\n')
            print(self.msg)
...
#
#     def __init__(self,get_UI):
#         self.file_name = get_UI
#
#     def entrance(self):
#         queue_log = Queue()  # 创建log队列
#         queue_status = Queue()  # 测试状态队列
#         test = ResultPush(queue_log)
#
#         datetime = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
#         file = os.path.basename(self.file_name).split('.')[0] + datetime
#         log = Logging(file + '.log',queue_log)
#         log_thread = Thread(target=log.customer, args=())
#         log_thread.start()
#         auto_test = TestPerform(self.file_name)
#         auto_test.case_run()
